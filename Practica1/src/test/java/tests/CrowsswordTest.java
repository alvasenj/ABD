...
public class CrowsswordTest {
	...
	public void dosCrucigramas() {
		...
		assertTrue(
				"La búsqueda por nombre con 'ruci' debe tener dos elementos como mínimo",
				l.size() >= 2);
		Set<String> titles = new HashSet<String>();
		for (Object x : l)
			titles.add(dao.getCrosswordTitle(x));

		assertTrue(
				"La búsqueda por nombre con 'ruci' debe devolver 'Crucigrama1' y 'Crucigrama2'",
				titles.contains("Crucigrama1")
						&& titles.contains("Crucigrama2"));
	}

	@Test
	public void crucigramasDeUser() {
		List<?> l = dao.findCrosswordsByTitle("Crucigrama2");
		assertTrue("Debe existir un crucigrama con título 'Crucigrama2'",
				l.size() >= 1);
		dao.addCrosswordToUser("user", l.get(0));
		List<?> cw = dao.getCrosswordsOf("user");
		assertTrue(
				"Tras añadir 'Crucigrama2' user debe tener al menos un crucigrama asociado",
				cw.size() >= 1);
		...
		assertTrue(
				"Tras añadir 'Crucigrama2' a 'user', éste debe estar en su lista de crucigramas",
				foundCW2);
	}
